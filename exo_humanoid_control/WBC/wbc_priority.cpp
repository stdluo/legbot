...
WBC_priority::WBC_priority(int model_nv_In, int QP_nvIn, int QP_ncIn, double mu_In, double dt) : QP_prob(QP_nvIn,
                                                                                                          QP_ncIn) {
    timeStep = dt;
    ...
    // TODO: Modify the torque constraints from fixed upper and lower bounds to dynamic bounds that account for tipping.
    tau_upp_stand_L << 5, 10, 20;    // foot end contact torque limit for stand state, in body frame
    tau_low_stand_L << -5, -10, -20;

    tau_upp_walk_L << 6, 15, 20; // foot end contact torque limit for walk state, in body frame
    tau_low_walk_L << -6, -15, -20;
    ...
    // taskOrder_walk.emplace_back("PxPyAbs");
    // taskOrder_walk.emplace_back("PosRot");
    taskOrder_walk.emplace_back("SwingLeg");
    // taskOrder_walk.emplace_back("SwingLegJoints");
    ...
    // taskOrder_stand.emplace_back("FeetParallel");
    // taskOrder_stand.emplace_back("Roll_Pitch_Yaw_Px_Py");
...
void WBC_priority::dataBusRead(const RobotState &robotState) {
    // deisred values
    ...
    swing_joint_des_q = robotState.swing_joint_des_q;
    ...
    if (legStateCur == RobotState::LSt) {
        ...
        swing_joint_q = robotState.q.block(11, 0, 4, 1);
    } else {
        Jc = robotState.J_r;
        dJc = robotState.dJ_r;
        Jsw = robotState.J_l;
        ...
        swing_joint_q = robotState.q.block(7, 0, 4, 1);
    }
...
void WBC_priority::dataBusWrite(RobotState &robotState) {
...
// QP_nv为优化变量的维度（包括delta_ddq(6) + delta_f(12)），QP_nc为约束条件的维度（包括等式约束(6) + 不等式约束(8*2)）
void WBC_priority::computeTau() {
    ...
    Eigen::Matrix3d Rfe;
    // QS: 站立时Rfe一直参考左脚的旋转矩阵fe_l_rot_cur_W，是否有些不妥？
    if (motionStateCur==RobotState::Stand){
        Rfe = fe_l_rot_cur_W;
    } else{
...
//    std::cout<<"wbc_computeTau, st_fe_rot"<<std::endl<<stance_fe_rot_cur_W<<std::endl;
    ...
    if (motionStateCur == RobotState::Walk) {
        if (legStateCur == RobotState::LSt) {
            f_upp(12) = 0;
            f_upp(13) = 0;
            f_upp(14) = 0;
            ...
            f_low(10) = -1e-7;
            f_low(11) = -1e-7;
        } else if (legStateCur == RobotState::RSt) {
            f_upp(4) = 0;
            f_upp(5) = 0;
            f_upp(6) = 0;
            f_upp(7) = 0;
    ...
    // Q1为外部接触力与期望接触力的误差权重，顺序为fl, tl, fr, tr
    // Q2为躯干加速度与期望加速度的误差权重
    Q2 = Eigen::MatrixXd::Identity(6, 6);
    Q1 = Eigen::MatrixXd::Identity(12, 12);
    eigen_qp_H.block<6, 6>(0, 0) = Q2 * 2.0 * 1e1; // 1e7
    // pure WBC test
    // eigen_qp_H.block<12, 12>(6, 6) = Q1 * 2.0 * 0;
    eigen_qp_H.block<12, 12>(6, 6) = Q1 * 2.0 * 1e1; //1e1
...
void WBC_priority::computeDdq() {
    // task definition
    /// -------- walk -------------
    {
        // 位置级WBC需要errX，速度级WBC需要dxDes，加速度级WBC需要ddxcmd，此外需要雅各比矩阵J，雅各比导数矩阵dJ，权重矩阵W
        // ddxcmd = ddxDes + kp * errX + kd * derrX
        // 由于规划器中没有加速度规划ddxDes，故ddxDes = Eigen::VectorXd::Zero
...
//            kin_tasks_walk.taskLib[id].dJ.block(0,22,3,3).setZero();
        ...
        // // 冗余关节任务（让腰部关节和头部关节保持固定姿态）
        // // 由于全是在关节空间中的控制，因此雅各比矩阵相当于一个对应项的单位矩阵
        ...
        // 通过taskMap将J_base映射到需要的雅各比矩阵（Roll_Pitch_Yaw_Pz）
        taskMap(0, 3) = 1;
        taskMap(1, 4) = 1;
        taskMap(2, 5) = 1;
        taskMap(3, 2) = 1;
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(2, 2) * 2000; //100
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(2, 2) * 500;
        ...
        id = kin_tasks_walk.getId("PxPyAbs");
        kin_tasks_walk.taskLib[id].errX = base_pos_des.block(0, 0, 2, 1) - q.block(0, 0, 2, 1);
        // kin_tasks_walk.taskLib[id].derrX = des_dq.block(0, 0, 2, 1) - dq.block(0, 0, 2, 1);
        kin_tasks_walk.taskLib[id].derrX = Eigen::VectorXd::Zero(2);
        kin_tasks_walk.taskLib[id].ddxDes = Eigen::VectorXd::Zero(2);
        kin_tasks_walk.taskLib[id].dxDes = Eigen::VectorXd::Zero(2);
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(2, 2) * 500; //100
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(2, 2) * 20;
        taskMap = Eigen::MatrixXd::Zero(2, 6);
        taskMap(0, 0) = 1;
        taskMap(1, 1) = 1;
        kin_tasks_walk.taskLib[id].J = taskMap * J_base;
        ...
        std::cout << "base_pos_des: " << base_pos_des.transpose() << std::endl;
        std::cout << "base_pos: " << q.block(0,0,3,1).transpose() << std::endl;
        // 如果posX和posY误差大于阈值，则截断误差
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) * 5;
        kin_tasks_walk.taskLib[id].kp.block(3,3,3,3)=Eigen::MatrixXd::Identity(3, 3) * 500; // 2000
        // kin_tasks_walk.taskLib[id].kp(0,0) = 0;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 2;
        kin_tasks_walk.taskLib[id].kd.block(3,3,3,3)=Eigen::MatrixXd::Identity(3, 3) * 20; // 100
        kin_tasks_walk.taskLib[id].J = J_base;
        kin_tasks_walk.taskLib[id].dJ = dJ_base;
        kin_tasks_walk.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
        std::cout << "PosRot errX: " << kin_tasks_walk.taskLib[id].errX.transpose() << std::endl;

        // 笛卡尔空间的摆动相控制任务
        // TODO: 可以考虑通过逆运动学转为关节空间的控制任务
        id = kin_tasks_walk.getId("SwingLeg");
        kin_tasks_walk.taskLib[id].errX = Eigen::VectorXd::Zero(6);
        kin_tasks_walk.taskLib[id].errX.block<3, 1>(0, 0) = swing_fe_pos_des_W - fe_pos_sw_W;
        std::cout << "swing_fe_pos_des_W: " << swing_fe_pos_des_W.transpose() << std::endl;
        std::cout << "fe_pos_sw_W: " << fe_pos_sw_W.transpose() << std::endl;
        ...
        // 关节空间的摆动相控制任务
        id = kin_tasks_walk.getId("SwingLegJoints");
        kin_tasks_walk.taskLib[id].errX = Eigen::VectorXd::Zero(4);
        kin_tasks_walk.taskLib[id].errX = swing_joint_des_q - swing_joint_q;
        std::cout << "swing_joint_des_q: " << swing_joint_des_q.transpose() << std::endl;
        std::cout << "swing_joint_q: " << swing_joint_q.transpose() << std::endl;
        kin_tasks_walk.taskLib[id].derrX = Eigen::VectorXd::Zero(4);
        kin_tasks_walk.taskLib[id].ddxDes = Eigen::VectorXd::Zero(4);
        kin_tasks_walk.taskLib[id].dxDes = Eigen::VectorXd::Zero(4);
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(4, 4) * 1000;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(4, 4) * 5;
        kin_tasks_walk.taskLib[id].J = Eigen::MatrixXd::Zero(4,model_nv);
        if (legStateCur == RobotState::LSt){
            kin_tasks_walk.taskLib[id].J.block(0, 10, 4, 4) = Eigen::MatrixXd::Identity(4, 4);
        }
        else{
            kin_tasks_walk.taskLib[id].J.block(0, 6, 4, 4) = Eigen::MatrixXd::Identity(4, 4);
        }
        kin_tasks_walk.taskLib[id].dJ = Eigen::MatrixXd::Zero(4,model_nv);
        kin_tasks_walk.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
        std::cout << "SwingLegJoints errX: " << kin_tasks_walk.taskLib[id].errX.transpose() << std::endl;
...
//        kin_tasks_stand.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
        ...
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(1, 1) * 0; //2000
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(1, 1) * 0;
        ...
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(2, 2) * 500; //2000
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(2, 2) * 25;
...
//        std::cout<<"pCoMDes"<<std::endl<<pCoMDes.transpose()<<std::endl;
        ...
        id = kin_tasks_stand.getId("Roll_Pitch_Yaw_Px_Py");
        kin_tasks_stand.taskLib[id].errX = Eigen::VectorXd::Zero(5);
        Eigen::Matrix3d desRot = eul2Rot(base_rpy_des(0), base_rpy_des(1), base_rpy_des(2));
        kin_tasks_stand.taskLib[id].errX.block<3, 1>(0, 0) = diffRot(base_rot, desRot);
        kin_tasks_stand.taskLib[id].errX.block<2,1>(3,0) = base_pos_des.block<2,1>(0,0) - q.block<2,1>(0,0);
        kin_tasks_stand.taskLib[id].derrX = Eigen::VectorXd::Zero(5);
        kin_tasks_stand.taskLib[id].derrX.block<3, 1>(0, 0) = -dq.block<3, 1>(3, 0);
        kin_tasks_stand.taskLib[id].derrX.block<2,1>(3,0) = Eigen::VectorXd::Zero(2) - dq.block<2,1>(0,0);
        kin_tasks_stand.taskLib[id].ddxDes = Eigen::VectorXd::Zero(5);
        kin_tasks_stand.taskLib[id].dxDes = Eigen::VectorXd::Zero(5);
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(5, 5) * 0;
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(5, 5) * 0;
        taskMap = Eigen::MatrixXd::Zero(5, 6);
        taskMap(0, 3) = 1;
        taskMap(1, 4) = 1;
        taskMap(2, 5) = 1;
        taskMap(3, 0) = 1;
        ...
        kin_tasks_stand.taskLib[id].dJ = taskMap * dJ_base;
        kin_tasks_stand.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
        // std::cout << "errX: " << std::endl << kin_tasks_stand.taskLib[id].errX.transpose() << std::endl;
        // std::cout << "derrX: " << std::endl << kin_tasks_stand.taskLib[id].derrX.transpose() << std::endl;
        // std::cout << "J_base: " << std::endl << J_base << std::endl;
        ...
        // std::cout << "stand base_pos_des: " << base_pos_des.transpose() << std::endl;
        // std::cout << "stand base_pos: " << q.block(0,0,3,1).transpose() << std::endl;
        // 如果posX和posY误差大于阈值，则截断误差
        ...
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) * 200;
        kin_tasks_stand.taskLib[id].kp.block(3,3,3,3)=Eigen::MatrixXd::Identity(3, 3) * 125; // 2000
        // kin_tasks_stand.taskLib[id].kp(0,0) = 0;
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 5;
        kin_tasks_stand.taskLib[id].kd.block(3,3,3,3)=Eigen::MatrixXd::Identity(3, 3) * 25; //100
        kin_tasks_stand.taskLib[id].J = J_base;
        kin_tasks_stand.taskLib[id].dJ = dJ_base;
        kin_tasks_stand.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
        // std::cout << "stand PosRot errX: " << kin_tasks_stand.taskLib[id].errX.transpose() << std::endl;

        id = kin_tasks_stand.getId("FeetParallel");
        kin_tasks_stand.taskLib[id].errX = Eigen::VectorXd::Zero(6);
        desRot = eul2Rot(0, 0, 0);
        kin_tasks_stand.taskLib[id].errX.block<3, 1>(0, 0) = diffRot(fe_l_rot_cur_W, desRot);
        kin_tasks_stand.taskLib[id].errX.block<3, 1>(3, 0) = diffRot(fe_r_rot_cur_W, desRot);
        std::cout << "stand FeetParallel errX: " << kin_tasks_stand.taskLib[id].errX.transpose() << std::endl;
        kin_tasks_stand.taskLib[id].derrX = Eigen::VectorXd::Zero(6);
        kin_tasks_stand.taskLib[id].ddxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_stand.taskLib[id].dxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) * 0;
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 0;
        Eigen::MatrixXd J_feet_paral = Eigen::MatrixXd::Zero(6, model_nv); // jacobian of feet rot
        Eigen::MatrixXd dJ_feet_paral = Eigen::MatrixXd::Zero(6, model_nv); // jacobian of feet rot
        J_feet_paral.block(0, 0, 3, model_nv) = Jfe.block(3, 0, 3, model_nv);
        J_feet_paral.block(3, 0, 3, model_nv) = Jfe.block(9, 0, 3, model_nv);
        dJ_feet_paral.block(0, 0, 3, model_nv) = dJfe.block(3, 0, 3, model_nv);
        dJ_feet_paral.block(3, 0, 3, model_nv) = dJfe.block(9, 0, 3, model_nv);
        // std::cout << "J_feet_paral: " << std::endl << J_feet_paral << std::endl;
        // std::cout << "dJ_feet_paral: " << std::endl << dJ_feet_paral << std::endl;
        kin_tasks_stand.taskLib[id].J = J_feet_paral;
        kin_tasks_stand.taskLib[id].dJ = dJ_feet_paral;
        kin_tasks_stand.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
...
//         kin_tasks_stand.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
        ...
        kin_tasks_stand.taskLib[id].ddxDes = Eigen::VectorXd::Zero(3);
        kin_tasks_stand.taskLib[id].dxDes = Eigen::VectorXd::Zero(3);
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(3, 3) * 125;
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(3, 3) * 25;
    ...
    if (motionStateCur==RobotState::Walk) {
        ...
        std::cout << "WBC_kin_tasks_walk" << std::endl;
    }
    else if (motionStateCur==RobotState::Stand) {
        // std::cout << "WBC_kin_tasks_stand_in" << std::endl;
        ...
        std::cout << "WBC_kin_tasks_stand" << std::endl;
    } else
    {
        delta_q_final_kin = Eigen::VectorXd::Zero(model_nv);
        dq_final_kin = Eigen::VectorXd::Zero(model_nv);
