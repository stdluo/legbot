...
//
#ifndef WBC_PRIORITY_H
#define WBC_PRIORITY_H

#include "../third_party/qpOASES/include/qpOASES.hpp"
#include <algorithm>
#include <Eigen/Dense>
#include "../include/common/RobotState.h"
...
public:
    ...
    RobotState::LegState legStateCur;
    RobotState::MotionState motionStateCur;
    WBC_priority(int model_nv_In, int QP_nvIn, int QP_ncIn, double mu_In, double dt);
    double mu{0.5};
    ...
    void computeTau();
    void dataBusRead(const RobotState &robotState);
...
private:
    ...
    Eigen::Vector3d base_pos_des, base_pos, base_rpy_des, base_rpy_cur;
    Eigen::Matrix3d base_rot;
    Eigen::VectorXd swing_fe_pos_des_W, swing_fe_rpy_des_W;
    Eigen::VectorXd swing_joint_des_q, swing_joint_q;
