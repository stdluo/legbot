                                                                                                 ...
                                                                                                 _nIterations(nMPC_segments)
{
  ...
  _stance = durations;                
  _swing << nMPC_segments - durations[0], nMPC_segments - durations[1]; 
}
...
// Generate and return the MPC gait table.
int *Gait::mpc_gait(Gait* next_gait)
{
  for (int i = 0; i < _nIterations; i++)
  {
    int iter = i + _iteration;
    if(iter < _nIterations)
    {
      Array2i progress = iter - _offsets; // 0 5
      for (int j = 0; j < 2; j++)
      {
    ...
    {
      int next_iter = iter - _nIterations;
      Array2i next_progress = next_iter - next_gait->_offsets; // 0 5
      for (int j = 0; j < 2; j++)
      {
        if (next_progress[j] < 0)
          next_progress[j] += _nIterations;
        if (next_progress[j] < next_gait->_durations[j])
          _mpc_table[i * 2 + j] = 1;
        else
          _mpc_table[i * 2 + j] = 0;
      }
