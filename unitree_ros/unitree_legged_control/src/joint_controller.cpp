...
namespace unitree_legged_control 
{
    ...
    void UnitreeJointController::update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        calcTorque = servoCmd.torque + servoCmd.posStiffness*(servoCmd.pos - currentPos) + servoCmd.velStiffness*(servoCmd.vel - currentVel);

        // std::cout <<joint_name<< " tauOri:" << calcTorque << std::endl;   
        // std::cout <<joint_name<< " tauCmd:" << servoCmd.torque << std::endl; 
        effortLimits(calcTorque);

        joint.setCommand(calcTorque);

        // std::cout <<joint_name<< " tau:" << calcTorque << std::endl;
